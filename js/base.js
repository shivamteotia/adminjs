...
function render(element, template, data, myHelper, cb) {
    // $(element).html('');
    // $.tmpl(template, data).appendTo(element);
    var xx = $.templates("#" + template);
    xx.link(element, data, myHelper);
    //element).html(.render(data, myHelper));
    if (cb)
        cb();
}
...
function rb(element, template, data, button, myHelper, cb, rcb) {
    render(element, template, data, myHelper, function() {
